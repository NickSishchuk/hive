...
public class SessionService {
    ...
    public StartSessionResponse startSession(User user) {
        // If an active session exists, return it instead of killing it
        var existingSession = sessionRepository.findByUserAndEndedAtIsNull(
            user
        );
        if (existingSession.isPresent()) {
            var session = existingSession.get();
            String livekitToken = generateLiveKitToken(
                user,
        ...
        // Only create a new session if none is active
    ...
    public EndSessionResponse endSession(
        ...
        if (!session.isActive()) throw new IllegalStateException(
    ...
    @Transactional
    public void updatePomodoroCount(
        UUID sessionId,
        User user,
        UpdatePomodoroRequest request
    ) {
        Session session = sessionRepository
            .findByIdAndUser(sessionId, user)
            .orElseThrow(() ->
    ...
    public StatsResponse getStats(User user) {
        ...
        Instant todayStart = now
            .toLocalDate()
            .atStartOfDay()
            .atZone(ZoneId.systemDefault())
            .toInstant();
        Instant weekAgo = now.minusDays(7).toInstant();
        int todayMinutes = sessionRepository.sumMinutesByUserSince(
            user,
            todayStart
        );
        int weekMinutes = sessionRepository.sumMinutesByUserSince(
            user,
            weekAgo
        );
